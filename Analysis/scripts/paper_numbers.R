...
df <- read.csv(file.path(root,"Analysis/processed/event_firm_panel.csv"), stringsAsFactors=FALSE, check.names=FALSE)
av <- read.csv(file.path(root,"Analysis/processed/event_firm_abnormal_volume.csv"), stringsAsFactors=FALSE)
...
b <- df[df$is_main_nasdaq100=="True" & df$event_excluded_identity=="False" &
...
b$related <- as.numeric(b$rel_upstream_hardware==1|b$rel_upstream_cloud==1|b$rel_downstream_integrator==1|
                        b$rel_downstream_deployer==1|b$rel_competitor==1|b$rel_is_investor==1|b$rel_is_owner==1)
POS0 <- "rel_upstream_hardware+rel_upstream_cloud+rel_downstream_integrator+rel_downstream_deployer+rel_competitor+rel_is_investor+rel_is_owner"
...
write.csv(res, file.path(root,"Analysis/reports/paper_numbers.csv"), row.names=FALSE)
cat("rows:", nrow(res), "\n")
