...
rel <- read.csv("output/data/specr_rel_clean.csv", stringsAsFactors = FALSE, check.names = FALSE)
...
df <- df %>%
  mutate(
    ...
    pos_exposure = pmax(owner, investor, cloud, real_upstream, real_downstream, na.rm = TRUE),
    broad_upstream = pmax(business_upstream, real_upstream, na.rm = TRUE),
    broad_downstream = pmax(business_downstream, real_downstream, na.rm = TRUE),
    any_relationship = pmax(owner, investor, cloud, business_upstream, real_upstream,
                            business_downstream, real_downstream, competitor, na.rm = TRUE)
  )
...
write.csv(capability_rows, file.path(out_dir, "proposal_capability_mechanisms.csv"), row.names = FALSE)

relation_terms <- c("owner", "investor", "cloud", "real_upstream", "real_downstream",
...
write.csv(interaction_rows, file.path(out_dir, "proposal_relationship_interactions.csv"), row.names = FALSE)
...
joint_specs <- list(
  ...
  "relationship bundle" = c("z_intelligence", "owner", "investor", "cloud", "real_upstream",
                            "real_downstream", "competitor")
)
joint_rows <- list()
...
write.csv(joint_rows, file.path(out_dir, "proposal_joint_models.csv"), row.names = FALSE)
...
write.csv(time_rows, file.path(out_dir, "proposal_time_learning_interactions.csv"), row.names = FALSE)
...
write.csv(roll_rows, file.path(out_dir, "proposal_rolling_event_window.csv"), row.names = FALSE)
...
write.csv(event_summary, file.path(out_dir, "proposal_gap_data_availability.csv"), row.names = FALSE)

topline <- capability_rows %>%
  filter(outcome == "mkt_car_20") %>%
  arrange(p_value) %>%
  mutate(across(c(coef, se, p_value, ci_lo, ci_hi, r_squared), ~ round(.x, 5)))
write.csv(topline, file.path(out_dir, "proposal_topline_capability_car20.csv"), row.names = FALSE)

cat("Supplement analysis complete.\n")
