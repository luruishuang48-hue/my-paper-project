...
## ============================================================================
## v2: re-coded relationship schema (8-dim) replacing the OLD 8-column coding.
##
## OLD columns (removed): owner, investor, cloud, business_upstream,
##   real_upstream, business_downstream, real_downstream, competitor
## NEW columns (data/panel/specr_rel_clean.csv): upstream_hardware,
##   upstream_cloud, downstream_integrator, downstream_deployer,
##   downstream_enabler, competitor, is_investor, is_owner
##
## Remapping logic applied below (see data/relationships/relationship_codebook.md):
##   owner            -> is_owner            (near 1:1)
##   investor         -> is_investor         (near 1:1, slightly broader)
##   cloud            -> upstream_cloud      (broader; now also captures
##                                            cloud exposure independent of
##                                            who happens to be a competitor)
##   broad_upstream   = pmax(business_upstream, real_upstream)
##                   -> pmax(upstream_hardware, upstream_cloud)
##                      (old upstream was split hardware vs. "real" cloud/
##                       compute; new schema cleanly separates hardware
##                       suppliers (R1) from cloud infra (R2), so the union
##                       of both is the direct semantic successor of the
##                       old broad upstream construct)
##   broad_downstream = pmax(business_downstream, real_downstream)
##                   -> pmax(downstream_integrator, downstream_deployer,
##                           downstream_enabler)
##                      (old downstream was a 2-way business/real split;
##                       new schema is a 3-way mutually-exclusive split by
##                       depth of AI dependence (R3/R4/R5). The union of all
##                       three is the semantic successor of "any downstream
##                       AI exposure")
##   positive_exposure = pmax(owner, investor, cloud, real_upstream, real_downstream)
##                   -> pmax(is_owner, is_investor, upstream_cloud,
##                           upstream_hardware, downstream_integrator)
##                      (old positive_exposure deliberately excluded the
##                       *weak* "business_" links and kept only owner/
##                       investor/cloud plus the "real" -- i.e. economically
##                       substantive -- upstream/downstream ties. The new
##                       schema's closest analog to "real" (substantive,
##                       core-business) exposure is downstream_integrator
##                       (R3: AI is a *core* product input), as opposed to
##                       downstream_deployer/enabler which are the weaker,
##                       supplementary/intermediary forms (analogous to the
##                       old "business_" links). Hardware and cloud are
##                       structurally substantive on the upstream side, so
##                       both are included, mirroring how upstream had no
##                       business/real distinction problem on this axis in
##                       the old positive_exposure construct.)
##   any_relationship = pmax of all 8 old flags
##                   -> pmax of all 8 new flags (straightforward: "any
##                      relationship of any kind, however weak")
##   competitor       -> competitor          (same name; new version is
##                                            slightly broader; all old
##                                            1s preserved as new 1s)
##
## downstream_subrole detail table: old version cross-tabbed by
##   real_downstream x business_downstream (2x2, since these could overlap).
##   New schema's three downstream categories are *designed* to be mutually
##   exclusive (codebook section 3, note after Step 8), so v2 simply tabs by
##   whichever of downstream_integrator / downstream_deployer /
##   downstream_enabler is flagged (falling back to "multiple_downstream" in
##   the rare case more than one is 1, and "other_downstream" if somehow none
##   are 1 despite broad_downstream==1, which should not occur but is kept
##   defensively).
##
## All output filenames get a _v2 suffix so original v1 outputs are untouched.
## ============================================================================
...
rel_vars <- c(
  "final_event_id", "company", "upstream_hardware", "upstream_cloud",
  "downstream_integrator", "downstream_deployer", "downstream_enabler",
  "competitor", "is_investor", "is_owner", "relationship_confidence",
  "relationship_justification", "relationship_notes"
)
...
df <- df %>%
  mutate(
    ...
    positive_exposure = safe_pmax(
      is_owner, is_investor, upstream_cloud, upstream_hardware, downstream_integrator
    ),
...
role_defs <- tribble(
  ~role, ~role_label, ~var,
  "is_owner", "发布方或所有者", "is_owner",
  "is_investor", "主要投资者", "is_investor",
  "upstream_cloud", "云服务方", "upstream_cloud",
  "upstream", "上游(硬件+云)", "broad_upstream",
...
owner_values <- df %>% filter(is_owner == 1)
...
write.csv(role_summary, file.path(out_dir, "role_car_mean_tests_v2.csv"), row.names = FALSE)

downstream_detail <- df %>%
  filter(broad_downstream == 1) %>%
  mutate(
    n_downstream_labels = downstream_integrator + downstream_deployer + downstream_enabler,
    downstream_subrole = case_when(
      n_downstream_labels > 1 ~ "multiple_downstream",
...
write.csv(downstream_detail, file.path(out_dir, "downstream_subrole_car_tests_v2.csv"), row.names = FALSE)
...
write.csv(sentiment_results, file.path(out_dir, "sentiment_asymmetry_disagreement_results_v2.csv"), row.names = FALSE)

role_sent_rows <- list()
for (role_var in c("is_owner", "broad_upstream", "broad_downstream", "competitor", "upstream_cloud", "is_investor")) {
...
write.csv(role_sent_results, file.path(out_dir, "sentiment_relationship_interactions_v2.csv"), row.names = FALSE)
...
write.csv(mechanism_results, file.path(out_dir, "cost_efficiency_capability_results_v2.csv"), row.names = FALSE)
...
write.csv(event_feature_summary, file.path(out_dir, "proposal_data_availability_update_v2.csv"), row.names = FALSE)

role_map <- tribble(
  ~proposal_role, ~current_proxy, ~interpretation,
  "publisher", "is_owner", "当前数据把发布方或所有者合并在 is_owner 中，不能严格区分上市发布者和母公司。",
  "parent_company", "is_owner", "若模型发布者是上市公司控股主体，最接近 is_owner。",
  "strategic_partner", "is_investor/upstream_cloud/relationship_notes", "当前没有独立 strategic_partner dummy，只能由投资方、云服务方和备注近似。",
  "major_investor", "is_investor", "主要投资者已有独立标记（新版定义略宽）。",
  "cloud_provider", "upstream_cloud", "云服务方已有独立标记（新版定义明显更宽）。",
  "distribution_partner", "relationship_notes", "没有独立 dummy，需要人工从关系备注重新编码。",
  "direct_competitor", "competitor", "直接竞争者已有独立标记（新版定义略宽，旧版1全部保留为新版1）。",
  "compute_supplier", "upstream_hardware/upstream_cloud", "上游算力现由硬件供应商和云服务商两个独立维度共同近似（替代旧版 business_upstream/real_upstream 的二分）。",
  "application_exposed_firm", "downstream_integrator/downstream_deployer/downstream_enabler", "应用层暴露现由三个互斥的下游维度（核心集成商/部署者/赋能者）共同近似（替代旧版 business_downstream/real_downstream 的二分）。",
...
write.csv(role_map, file.path(out_dir, "proposal_to_current_role_mapping_v2.csv"), row.names = FALSE)
...
write.csv(influence_results, file.path(out_dir, "influence_robustness_results_v2.csv"), row.names = FALSE)
...
write.csv(loo_results, file.path(out_dir, "leave_one_event_out_intelligence_v2.csv"), row.names = FALSE)
...
png(file.path(out_dir, "figure_role_car20_means_v2.png"), width = 1800, height = 1100, res = 180)
ggplot(role_plot, aes(x = role_label, y = mean)) +
  ...
  coord_flip() +
  labs(x = NULL, y = "Mean CAR[0,+20]", title = "CAR[0,+20] by event-firm exposure role (v2 schema)") +
...
png(file.path(out_dir, "figure_sentiment_asymmetry_car20_v2.png"), width = 1500, height = 1000, res = 180)
ggplot(sent_plot, aes(x = term_label, y = coef)) +
  ...
  labs(x = NULL, y = "Coefficient on CAR[0,+20]", title = "Asymmetric media sentiment association (v2 schema)") +
  theme_minimal(base_size = 12)
dev.off()

cat("Missing analyses complete (v2 relationship schema).\n")
