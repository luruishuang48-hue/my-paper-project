#!/usr/bin/env Rscript
# =============================================================================
# relonly_regression.R
# 剔除无关系标签行（relationship IS NULL）后重跑全套主要回归
# 全样本: 5160行 → 有关系行: 3143行（61%）
# 输出: outputs/r_XX_*.csv（前缀r_表示rel-only版本）
# =============================================================================
...
# ─── 加载数据 ─────────────────────────────────────────────────────────────────
df_all <- read.csv("outputs/specr_621_clean.csv", stringsAsFactors = FALSE,
                   check.names = FALSE)
...
write.csv(tbl_main, "outputs/r_01_main_regression.csv", row.names=FALSE)
cat("-> 已保存: outputs/r_01_main_regression.csv\n")
...
write.csv(tbl_rel, "outputs/r_02_relationship_subsamples.csv", row.names=FALSE)
cat("\n-> 已保存: outputs/r_02_relationship_subsamples.csv\n")
...
if (length(rows_ind) > 0) {
  write.csv(do.call(rbind,rows_ind), "outputs/r_03_industry.csv", row.names=FALSE)
  cat("-> 已保存: outputs/r_03_industry.csv\n")
}
...
write.csv(do.call(rbind,rows_vix), "outputs/r_04_vix_moderator.csv", row.names=FALSE)
cat("-> 已保存: outputs/r_04_vix_moderator.csv\n")
...
write.csv(do.call(rbind,rows_news), "outputs/r_05_news_moderator.csv", row.names=FALSE)
cat("-> 已保存: outputs/r_05_news_moderator.csv\n")
...
write.csv(do.call(rbind,rows_oc), "outputs/r_06_open_vs_closed.csv", row.names=FALSE)
cat("-> 已保存: outputs/r_06_open_vs_closed.csv\n")
...
write.csv(do.call(rbind,rows_ff3), "outputs/r_07_ff3_robustness.csv", row.names=FALSE)
cat("-> 已保存: outputs/r_07_ff3_robustness.csv\n")
...
write.csv(do.call(rbind,rows_mag), "outputs/r_08_mag7.csv", row.names=FALSE)
cat("-> 已保存: outputs/r_08_mag7.csv\n")
...
old <- read.csv("outputs/01_main_regression.csv")
new <- tbl_main
# 全样本
