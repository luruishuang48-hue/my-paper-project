"""
预处理（v2，含仲裁后新关系列）：task/事件集数据-relationships_v2.csv → data/panel/specr_rel_clean_v2.csv (UTF-8)
镜像 specr_rel_prep.py 的清洗逻辑，仅替换输入/输出路径并扩展数值化列表。
"""
import pandas as pd
import numpy as np

df = pd.read_csv('task/事件集数据-relationships_v2.csv', encoding='utf-8', dtype=str)
...
# 旧关系列保持原名: owner, investor, cloud,
# business_upstream, real_upstream, business_downstream, real_downstream, competitor
# 新关系列（来自仲裁结果）: upstream_hardware, upstream_cloud, downstream_integrator,
# downstream_deployer, downstream_enabler, competitor_new, is_investor, is_owner,
# relationship_confidence, relationship_justification
...
num_cols = [
    ...
    'downstream_deployer','downstream_enabler','competitor_new',
...
df.to_csv('data/panel/specr_rel_clean_v2.csv', index=False, encoding='utf-8')
print(f"Saved: {len(df)} rows x {len(df.columns)} cols")
print(f"Events: {df['final_event_id'].nunique()}")

print(f"\nOld relationship counts (=1):")
...
print(f"\nNew (adjudicated) relationship counts (=1):")
for c in ['upstream_hardware','upstream_cloud','downstream_integrator',
          'downstream_deployer','downstream_enabler','competitor_new',
          'is_investor','is_owner']:
    n = int(df[c].sum()) if c in df.columns else 'N/A'
    print(f"  {c}: {n}")
