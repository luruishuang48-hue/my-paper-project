...
df_rel <- read.csv("output/data/specr_rel_clean.csv", stringsAsFactors = FALSE, check.names = FALSE)
df_rel <- df_rel[!is.na(df_rel$final_event_id) & df_rel$final_event_id != "" &
                   !is.na(df_rel$company_id) & df_rel$company_id != "", ]
rel_vars <- c("owner", "investor", "cloud", "business_upstream", "real_upstream",
...
num_cols <- c(
  ...
  "is_open_weight", "owner", "investor", "cloud", "business_upstream",
...
write_csv(core_results, file.path(out_dir, "core_robustness_results.csv"))
...
write_csv(event_results, file.path(out_dir, "event_level_results.csv"))
write_csv(ed_all, file.path(out_dir, "event_level_dataset_all_firms.csv"))
write_csv(ed_rel, file.path(out_dir, "event_level_dataset_related_firms.csv"))
...
write_csv(loo_results, file.path(out_dir, "leave_one_creator_out_results.csv"))
...
overlap <- event_dates %>%
  ...
  ungroup()
write_csv(overlap, file.path(out_dir, "event_window_overlap_diagnostic.csv"))
...
write_csv(no_overlap_results, file.path(out_dir, "no_overlap_results.csv"))
...
report <- c(
  "# Econometric Robustness Report",
  "",
  "本报告由 `run_econometric_robustness.R` 自动生成。",
  "",
  "## 样本摘要",
  "",
...
writeLines(report, file.path(out_dir, "econometric_robustness_report.md"))
