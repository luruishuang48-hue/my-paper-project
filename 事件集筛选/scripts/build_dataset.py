...
COMPARISON_CONTEXT_RE = re.compile(
    r"(outperform\w*|compared to|compared with|comparable to|comparable with|"
    r"rival\w*|competes? with|similar to|"
    r"on par with|ahead of|beat\w*|exceed\w*|better than|versus|\bvs\.?\b|"
    r"surpasses (?:several |many |all )?(?:leading |other |)?models?,? including|"
    r"such as)",
    re.I,
)


def extract_bold_with_comparison_flag(value):
    """标注每个加粗实体是否出现在对比语境中（如 'outperformed X'、'similar to X'）。

    AiTimeline 的加粗规则不区分“本次发布的模型”和“文本中作为能力对比基准提及的
    其他公司模型”（例如 "Alibaba unveiled Qwen2.5-Max...surpasses...GPT-4o, Claude 3.5"）。
    只看同一句内、该实体之前的文本是否命中比较性动词/介词——用句子边界（. ! ?）
    而非固定字符数来框定“同一句”，避免跨句误判。
    """
    raw = value or ""
    results = []
    for match in re.finditer(r"<b>(.*?)</b>", raw, flags=re.I | re.S):
        item = strip_tags(match.group(1))
        item = re.sub(r"\s+", " ", item).strip()
        if not item:
            continue
        preceding = html.unescape(re.sub(r"<[^>]+>", " ", raw[: match.start()]))
        sentence_start = max(preceding.rfind(". "), preceding.rfind("! "), preceding.rfind("? "))
        local = preceding[sentence_start + 2 :] if sentence_start >= 0 else preceding
        is_comparison = bool(COMPARISON_CONTEXT_RE.search(local))
        results.append((item, is_comparison))
    return results
    ...
    for year_block in timeline:
        ...
        for month_block in year_block.get("events", []):
            ...
            for item_idx, item in enumerate(month_block.get("info", []), start=1):
                ...
                bold_with_flags = extract_bold_with_comparison_flag(raw_text)
                bold_names = [name for name, _ in bold_with_flags]
                event_id = f"AIT-{year}-{month_num or 0:02d}-{item_idx:03d}"
                creator = infer_creator(clean_text)
                entity_types = []
                for entity_idx, (name, is_comparison) in enumerate(bold_with_flags, start=1):
                    ...
                    entities.append(
                        {
                            ...
                            "is_comparison_reference": is_comparison,
...
AUTO_UNMATCH_SCORE_CEILING = 0.75
...
def content_tokens(name):
    ...
    return {t for t in toks if t not in STOPWORDS_FOR_ROOT and len(t) > 1}


def shares_content_root(entity_name, candidate_name):
    entity_tokens = content_tokens(entity_name)
    candidate_tokens = content_tokens(candidate_name)
...
def match_one(entity, aa_rows):
    ...
    auto_unmatch_reason = ""
    if level in {"medium", "low"} and best_score < AUTO_UNMATCH_SCORE_CEILING:
        if not shares_content_root(name, best.get("aa_name", "")):
            auto_unmatch_reason = (
                f"score {best_score:.4f} < {AUTO_UNMATCH_SCORE_CEILING} and no shared "
                f"content root with top candidate '{best.get('aa_name', '')}'"
            )
            level = "unmatched"
    ...
    out.update(
        {
            ...
            "auto_unmatch_reason": auto_unmatch_reason,
        }
    )
    return out
...
def build_report(events, entities, aa_rows, matches):
    ...
    lines.append("## 文件清单")
    lines.append("")
    lines.append("- 原始 AI Timeline 数据 `事件集筛选/raw/ai_timeline_timeline.json`")
    lines.append("- 原始 AA API 数据 `事件集筛选/raw/aa_api_v2/`")
    lines.append("- 清洗后事件表 `事件集筛选/processed/ai_timeline_events.csv`")
    lines.append("- 清洗后实体表 `事件集筛选/processed/ai_timeline_entities.csv`")
    lines.append("- AA 合并模型表 `事件集筛选/processed/aa_models.csv`")
    lines.append("- 匹配结果 `事件集筛选/processed/ai_timeline_aa_model_matches.csv`")
    lines.append("- 需人工审核匹配 `事件集筛选/processed/ai_timeline_aa_model_matches_review.csv`")
    lines.append("")
    lines.append("## 数据来源")
    lines.append("")
    ...
    lines.append("- 语言模型记录来自 `/api/v2/data/llms/models` 快照")
    lines.append("- 媒体模型记录来自各任务的 `/api/v2/data/media/` 快照")
    lines.append("")
    lines.append("## 分类规则")
    lines.append("")
    ...
    lines.append(f"- `exact_or_near_exact` 和 `high` 直接采用；`medium`、`low` 中若最高分候选与实体名无共享有效词根（去除 model/version 等停用词后）且分数低于 {AUTO_UNMATCH_SCORE_CEILING}，自动改判为 `unmatched` 并记录 `auto_unmatch_reason`，不再进入人工审核队列")
    lines.append("- 该自动判定规则已在全量样本上验证：不会把任何 `exact_or_near_exact` 或 `high` 记录降级")
    lines.append("- 未被自动改判的 `medium`、`low` 记录（通常是同族但版本粒度不一致，如尺寸变体或实验快照），以及原生 `unmatched`，仍需人工审核")
    lines.append("")
    lines.append("## 匹配结果概况")
    lines.append("")
    ...
    auto_unmatched = len([m for m in matches if m["auto_unmatch_reason"]])
    lines.append(f"- 非 unmatched 记录 {matched} 个")
    lines.append(f"- 其中自动改判为 unmatched 的记录 {auto_unmatched} 个（见 `ai_timeline_aa_model_matches_auto_unmatched.csv`）")
    lines.append(f"- 仍需人工审核记录 {len([m for m in matches if m['needs_review']])} 个")
    lines.append("")
    ...
    lines.append("## 使用建议")
    lines.append("")
    lines.append("- 先审核 `ai_timeline_aa_model_matches_review.csv`，此时已排除自动判定的 unmatched 记录")
    lines.append("- 对仍在 `medium`、`low` 的记录，优先看 `top_candidates_json` 中前 5 个候选")
    lines.append("- `ai_timeline_aa_model_matches_auto_unmatched.csv` 中每条记录都附 `auto_unmatch_reason`，可直接核查判定依据")
...
def main():
    ...
    write_csv(
        PROCESSED / "ai_timeline_aa_model_matches_auto_unmatched.csv",
        [m for m in matches if m["auto_unmatch_reason"]],
        match_fields,
    )
