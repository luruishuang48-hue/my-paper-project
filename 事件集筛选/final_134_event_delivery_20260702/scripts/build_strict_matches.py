...
def parse_release_date(value):
    ...
    match = re.match(r"^([A-Za-z]{3,9})\.?\s*[-/ ]\s*'?\s*(\d{2}|\d{4})$", text)
    if match:
...
def candidate_sort_key(row):
    ...
    if parsed:
        date_key = (0, parsed[0], parsed[1], parsed[2])
    else:
        date_key = (1, 9999, 99, 99)
    return date_key + (
        row.get("aa_modality") or "",
        row.get("aa_creator") or "",
        row.get("aa_name") or "",
        row.get("aa_record_key") or row.get("aa_id") or row.get("aa_slug") or "",
    )
...
def build():
    ...
    lines = [
        ...
        "- 发布时间不再决定删留，只作为参考列保留。",
        "- 同月诊断会先把 AA 发布时间解析成年月，支持 `YYYY/M/D`、`YYYY-M-D`、`YYYY-M`、`Oct-24` 和 `October 2024` 等格式。",
